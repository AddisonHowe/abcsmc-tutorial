...
EPS0 = 5
EPS_PERCENTILE = 0.15
MODEL_PARAMS = [-5, 3]  # mean, variance
NDRAWS = 10

# Image output
imgdir = f"../../out/images/nb_gaussian_specified_{KERNEL_METHOD}"
...
if SCRIPT:
    ...
    if args.imgdir:
        ...
        imgdir = f"{args.outdir}/nb_gaussian_specified_{KERNEL_METHOD}"
...
prior_theta2 = UniformPrior(0, 20)
prior_list = [prior_theta1, prior_theta2]
plot_range = [[-20, 20], [0, 20]]

# True model and generated "observed" data
model = GaussianModel(mu, var, ndraws=ndraws)
...
# Simulation function
def f_sim(particle):
    m = GaussianModel(particle[0], particle[1], ndraws=ndraws)
