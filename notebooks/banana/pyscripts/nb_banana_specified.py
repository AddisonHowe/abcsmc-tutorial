...
# Image output
imgdir = f"../../out/images/nb_banana_specified_{KERNEL_METHOD}"
...
if SCRIPT:
    ...
    if args.imgdir:
        ...
        imgdir = f"{args.outdir}/nb_banana_specified_{KERNEL_METHOD}"
...
model = BananaModel(theta1, theta2, cov=cov_const, ndraws=ndraws)
data = model.generate_data(ndraws)

# Summary statistics of the observed data
data_mu = np.mean(data, axis=0)
data_cov = np.cov(data.T)
assert data_mu.shape == (2,)
assert data_cov.shape == (2,2)
...
# Distance function
def f_dist(x):
    mu_err = np.max(np.abs((np.mean(x, axis=0) - data_mu) / data_mu))
    cov_err = np.max(np.abs((np.cov(x.T) - data_cov) / data_cov))
    return mu_err + cov_err
